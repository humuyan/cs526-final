...
def generate_cuda_code(DATABASE, MODEL_FILENAME, tuning_records, WORK_DIR, TARGET):
    ...
    with open(path_lib + ".CUDAlaunch.config", "w") as outfile:
        # outfile.write(lowered_mod.__str__())
...
def profile_main(DEVICE, TARGET, RPC_CONFIG, onnx_path="", onnx_model=None, WORK_DIR="./tune_kernels/",
    ...
    if(DATABASE.has_workload(tir_mod)):
        ...
        # module = graph_executor.GraphModule(rt_mod2["default"](DEVICE))
        # print(f"Kernel {MODEL_FILENAME} has {module.get_num_inputs()} inputs")
        ...
        generate_cuda_code(DATABASE, MODEL_FILENAME, tuning_records, WORK_DIR, TARGET)
...
if __name__=="__main__":
    ...
    onnx_path = '962.onnx_onnx_0/1.onnx'
    ...
    profile_results = profile_main(DEVICE, TARGET, RPC_CONFIG, onnx_path=onnx_path, DB_WIPE=True,
