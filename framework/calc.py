...
from profiler import KernelProfilingManager
    ...
    def _dfs(self, depth: int, cur: set):
        for node in self.graph.nodes:
            ...
            for tensor in node.inputs:
                if not isinstance(tensor, gs.Constant) and tensor.name not in self.consts and tensor.name not in cur:
                    prepared = False
                    break
    ...
    def export_candidate_kernel_graph(self) -> Optional[gs.Graph]:
        ...
        for tensor_name in self.nodes:
            ...
            for tensor in cur_node.inputs:
                if not isinstance(tensor, gs.Constant) and tensor.name not in self.graph_analyzer.get_consts() and tensor.name not in self.nodes and tensor not in subgraph.inputs:
...
def main(input_args: argparse.Namespace, target_info: Target, config: GraphConfig) -> None:
    ...
    for subgraph_id, (inputs, outputs) in enumerate(config.cut_points):
        ...
        kernel_profiler = KernelProfilingManager(graph, target_info, kernel_profiler_database_path)
        candidate_kernels_latencies = kernel_profiler.profile_kernels(K, enable_trt=False)
        ...
        if input_args.code_output_dir is not None:
            ...
            print(f"selected_kernels: {[x[2] for x in selected_kernel_info]}")
